 ...
 */
import { Provider, ProviderOptions } from "@openauthjs/openauth/provider/provider"
import { SubjectPayload, SubjectSchema } from "@openauthjs/openauth/subject"
...
import { Storage, StorageAdapter } from "@openauthjs/openauth/storage/storage"
import { encryptionKeys, legacySigningKeys, signingKeys } from "@openauthjs/openauth/keys"
import { validatePKCE } from "@openauthjs/openauth/pkce"
...
>(input: IssuerInput<Providers, Subjects, Result>) {
  ...
  async function verifyToken(c: Context) {
    ...
    const validated = await input.subjects["user"]["~standard"].validate(result.payload.properties)
